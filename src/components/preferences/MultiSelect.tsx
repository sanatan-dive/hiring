    ...
    <div className={cn('relative', className)}>
      ...
      <div className="relative">
        <input
          ...
          className="w-full rounded-lg border border-gray-300 px-4 py-2 focus:border-transparent focus:ring-2 focus:ring-indigo-500 focus:outline-none"
        />

        {allowCustom && inputValue.trim() && !selected.includes(inputValue.trim()) && (
          <button
            onMouseDown={handleAddCustom}
            className="absolute top-1/2 right-2 -translate-y-1/2 transform p-1 text-indigo-600 hover:text-indigo-700"
          >
          ...
          {filteredOptions.map((option) => (
            ...
            >
              <div className="flex items-center justify-between">
                <span>{option}</span>
                {selected.includes(option) && <div className="h-4 w-4 rounded bg-indigo-600" />}
              </div>
            </div>
          ))}
